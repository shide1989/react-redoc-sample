...
1) Use `pnpm start` to start the React app.
2) Use `pnpm bundle:json` to bundle the OpenAPI files into a single JSON file (`src/bundle.json`) for the React application.
3) Use your brains to figure out how to modify the OpenAPI specification to suit your needs, or modify the theme 😈.
...
Runs the Redocly preview server, compiling your open API files.\
Open [http://localhost:8080](http://localhost:8080) to view it in the browser.

### `pnpm bundle:json`

Bundles the OpenAPI files into a single JSON file for the React application

### `pnpm bundle:yaml`

Bundles the OpenAPI files into a single YAML file at `dist/bundle.yaml`.

### `pnpm build:html`

Builds the OpenAPI files into a single HTML file for a static usage at `dist/bundle.html`.
...
If you have an JSON example, you can convert it to JSON schema using
Redocly's [JSON to JSON schema tool](https://redocly.com/tools/json-to-json-schema/).
...
2. Add a new YAML file named like your URL endpoint except replacing `/` with `_` (or whichever character you prefer)
...
Automated code sample generations is enabled in the Redocly configuration file. Add manual code samples by the following
process:
